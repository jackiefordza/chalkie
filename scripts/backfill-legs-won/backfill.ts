#!/usr/bin/env node
// Entry point — the Stats Rules audit (Season 1) legsWon backfill. Defaults
// to a read-only dry run; only --apply performs any write. See README.md.
...
function printBanner(mode: 'DRY RUN' | 'APPLY'): void {
  ...
  console.log('OPERATION:               Stats Rules audit — legsWon backfill');
  console.log(`MODE:                    ${mode}${mode === 'DRY RUN' ? ' (read-only — no writes)' : ' — WILL WRITE legsWon'}`);
  console.log('SCOPE:                   reads matches (status == confirmed) only;');
  console.log('                         writes ONLY the legsWon field on playerSeasonStats;');
  console.log('                         never touches matches, fixtures, or any other field.');
  console.log('='.repeat(60));
  console.log('');
}
...
async function main(): Promise<void> {
  ...
  if (!args.confirmBackfill) {
    throw new Error(
      ...
      + "chalkie-app's confirmed matches and compute/preview the legsWon backfill. Add --apply on top "
      + 'of that to actually write — omitting it always produces a dry run, never a write.',
    );
  }
  ...
  if (needsWrite.length === 0) {
    console.log('\nNothing to write — every playerSeasonStats document already has the correct legsWon.');
    return;
  }

  console.log(`\nApplying ${needsWrite.length} write(s)...`);
  const written = await applyPlan(db, plan);
  console.log(`\nWrote legsWon to ${written} playerSeasonStats document(s).`);
  console.log('\nAudit log:');
  getAuditLog().forEach((e) => console.log(`  ${e.docPath}: ${e.oldValue ?? '(field not set)'} -> ${e.newValue}`));
}
